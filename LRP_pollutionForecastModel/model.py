...
class EncoderDecoder(nn.Module):
    def __init__(self, in_seq_len, out_seq_len, in_feat_len,  out_feat_len, static_feat_len,  
        ...
        self.decoder_cell = DecoderCell(in_feat_len=in_feat_len ,
                                        ...
                                        hidden_size=hidden_size,
                                        static_feat_len=static_feat_len,
                                        dropout=dec_dropout, 
    ...
    def forward(self, enc_input, dec_input, static):
        ...
        # uncomment to null the encoder hidden state for testing
        # prev_hidden = torch.zeros(self.num_layers, enc_input.size(0), self.hidden_size, device=self.device).float()
...
class DecoderCell(nn.Module):
    def __init__(self, in_feat_len, out_feat_len, hidden_size, static_feat_len, dropout, num_layers, arch):
        super().__init__()
        ...
        self.out1 = nn.Linear(hidden_size + static_feat_len, hidden_size)
        self.relu = torch.nn.ReLU()
        self.out2 = nn.Linear(hidden_size, out_feat_len,bias=False)
        self.dropout = nn.Dropout(dropout)

    def forward(self, y, prev_hidden, static):
        output, hidden = self.decoder_rnn_cell(y, prev_hidden)
        output = torch.cat((output, static.unsqueeze(1)), axis=-1)
